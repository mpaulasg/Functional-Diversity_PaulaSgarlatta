...
kelp_sp_maxN_transect <- read.csv(here::here("from_paula", "TemporalBRUV_species_maxN_kelp.csv") ) %>%
  column_to_rownames("Code") %>%
  as.matrix()


# summary of surveys and occurrences data ----
kelp_summary <- mFD::asb.sp.summary(asb_sp_w = kelp_sp_maxN_transect)
...
# names of species
kelp_sp_transect <- colnames(kelp_sp_occ_transect) 
length(kelp_sp_transect) # 101 sp
...
spatial_sp_biom_transect <- read.csv(here::here("from_paula", "SpatialUVC_species_biomass_transect.csv")) %>%
  column_to_rownames("Code") %>% 
  as.matrix()

dim(spatial_sp_biom_transect) # 36 assemblages * 53 species

# summary of surveys and occurrences data ----
spatial_summary_transect <- mFD::asb.sp.summary(asb_sp_w = spatial_sp_biom_transect)

# retrieve occurrence matrix:
spatial_sp_occ_transect <- spatial_summary_transect$asb_sp_occ
...
## names of species present in at least one dataset ####
sum(spatial_sp_transect %in% kelp_sp_transect)# 35 species shared 


## saving dataframes #####

save(kelp_sp_occ_transect, file=here::here("data", "using biomass-maxN", "kelp_sp_occ_transect.RData") )
save(kelp_sp_maxN_transect, file=here::here("data", "using biomass-maxN", "kelp_sp_maxN_transect.RData") )

save(spatial_sp_occ_transect, file=here::here("data", "using biomass-maxN", "spatial_sp_occ_transect.RData") )
save(spatial_sp_biom_transect, file=here::here("data", "using biomass-maxN", "spatial_sp_biom_transect.RData") )
