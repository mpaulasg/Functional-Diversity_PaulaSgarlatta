...
kelp_interyear_taxo_turn_thermal <- dendextend::dist_long(temporal_beta_kelp_thermal$taxo_turn_thermal) %>%
  ...
  filter(Year1==2002 | Year2==2002) %>%
  mutate(Year = pmax ( Year1, Year2 ) ) %>%
  select(Year, taxo_turn_thermal = distance )  %>%
  group_by(Year) %>%
  summarise( n = n(),
             taxo_turn_mean = mean(taxo_turn_thermal),
             taxo_turn_sd = sd(taxo_turn_thermal)
  ) %>%
  mutate( taxo_turn_se = taxo_turn_sd/sqrt(n)) %>%
  mutate( Habitat = "Kelp", .before="Year" )
...
# merging ----
taxo_turnim_toplot_thermal <- bind_rows(kelp_interyear_taxo_turn_thermal, 
                                nokelp_interyear_taxo_turn_thermal,
                                spatial_turn_thermal) 

head(taxo_turnim_toplot_thermal)
...
plot_turnim_taxo_thermal <- ggplot(taxo_turnim_toplot_thermal, mapping = aes(color=Habitat, fill=Habitat)) +
  geom_point( aes(x=Year, y=taxo_turn_mean, 
                  shape= Habitat), stat="identity") +
  geom_errorbar( aes(x=Year, ymin=taxo_turn_mean-taxo_turn_se, ymax=taxo_turn_mean+taxo_turn_se), 
                 width=0.2, size=0.5 ) +
...
kelp_interyear_func_turn_thermal <- dendextend::dist_long(temporal_beta_kelp_thermal$func_turn_thermal) %>%
  ...
  filter(Year1==2002 | Year2==2002) %>%
  mutate(Year = pmax ( Year1, Year2 ) ) %>%
  select(Year, func_turn_thermal = distance )  %>%
  group_by(Year) %>%
  summarise( n = n(),
             func_turn_mean = mean(func_turn_thermal),
             func_turn_sd = sd(func_turn_thermal)
  ) %>%
  mutate( func_turn_se = func_turn_sd/sqrt(n)) %>%
  mutate( Habitat = "Kelp", .before="Year" )
...
# merging ----
func_turnim_toplot_thermal <- bind_rows(kelp_interyear_func_turn_thermal, 
                                nokelp_interyear_func_turn_thermal,
                                spatial_turn_thermal) 

head(func_turnim_toplot_thermal)

# plotting ----

plot_turnim_func_thermal <- ggplot(func_turnim_toplot_thermal, mapping = aes(color=Habitat, fill=Habitat)) +
  geom_point( aes(x=Year, y=func_turn_mean, 
                  shape= Habitat), stat="identity") +
  geom_errorbar( aes(x=Year, ymin=func_turn_mean-func_turn_se, ymax=func_turn_mean+func_turn_se), 
                 width=0.2, size=0.5) +
  ...
  scale_shape_manual(values=hab_shape) + 
  scale_x_continuous( limits = c(2002, 2018), breaks = seq(from=2002, to=2018, by=4)  ) +
  scale_y_continuous( limits = c(0.1,0.7), breaks = seq(from=0, to=1, by=0.2)  ) +
  labs(x="", y="Functional turnover") +
  theme(panel.background=element_rect(fill="white"), panel.grid.minor = element_blank(), axis.ticks.y = element_blank(), 
        panel.grid.major = element_blank(),axis.line = element_line(size = 1, colour = "black"),
        axis.text = element_text(size = (12),colour = "black"), axis.title = element_text(size= (14)),
        legend.title = element_text(size=12), legend.text = element_text(size=12),
        legend.background = element_blank(), legend.key=element_blank())
plot_turnim_func_thermal


#####
## merging all plot into a single figure and saving as png ####
figure2_turnover_thermal <- ( plot_turnim_taxo_thermal / plot_turnim_func_thermal )
ggsave(figure2_turnover_thermal, file=here::here("outputs/", "figure2_turnover_thermal.png"), 
       height = 20, width = 18, unit = "cm" )
