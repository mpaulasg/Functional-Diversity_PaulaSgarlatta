...
## [PS] As we are comparing 2002 with the rest of the years, I think is better to do comparisons between habitats too
...
################Graph of beta diversity diferent (only temporal) ######################
...
taxo_dissim_temporal_toplot <- bind_rows(kelp_interyear_taxo_diss, 
                                nokelp_interyear_taxo_diss) 
...
kelp_colors <- c(Kelp="#3BB372", No_Kelp="#74E7B8")

plot_dissim_temporal_taxo <- ggplot(taxo_dissim_temporal_toplot, aes(x=Year, y=distance_mean, color=Habitat)) +
  geom_point() +
  geom_smooth(method = "lm") +
  scale_color_manual(values=kelp_colors) + 
  scale_fill_manual(values=kelp_colors) + 
  scale_x_continuous( limits = c(2001, 2019), breaks = seq(from=2002, to=2018, by=4)  ) +
  #scale_y_continuous( limits = c(0.2,0.8), breaks = seq(from=0.2, to=0.8, by=0.2)  ) +
  labs(x="", y="Species dissimilarity") +
  theme(panel.background=element_rect(fill="white"), panel.grid.minor = element_blank(), axis.ticks.y = element_blank(), 
...
# merging ----
func_dissim_temporal_toplot <- bind_rows(kelp_interyear_func_diss, 
                                nokelp_interyear_func_diss) 
...
plot_dissim_temporal_func <- ggplot(func_dissim_temporal_toplot, aes( x=Year, y=distance_mean, color=Habitat)) +  
  geom_point() +
  geom_smooth(method = "lm") +
  scale_color_manual(values=kelp_colors) + 
  scale_fill_manual(values=kelp_colors) + 
