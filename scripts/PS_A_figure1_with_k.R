...
load(here::here("outputs/", "spatial_alpha_k.RData") )

load(here::here("data", "kelp_metadata.RData") )
load(here::here("data", "nokelp_metadata.RData") )
load(here::here("outputs/", "temporal_alpha_kelp_k.RData") )
load(here::here("outputs/", "temporal_alpha_nokelp_k.RData") )
...
temporal_kelp<- kelp_metadata %>% 
  mutate(Habitat="kelp") %>%
  left_join( rownames_to_column(temporal_alpha_kelp_k, "Code"), by="Code" ) %>%
  select(Code, Site, Year, Habitat, TRic=sp_richn, FRic=fric)

temporal_nokelp<- nokelp_metadata %>% 
  mutate(Habitat="no_kelp") %>%
  left_join( rownames_to_column(temporal_alpha_nokelp_k, "Code"), by="Code" ) %>%
  select(Code, Site, Year, Habitat, TRic=sp_richn, FRic=fric)
...
ggsave(figure1, file=here::here("outputs/", "figure1_k.png"),
