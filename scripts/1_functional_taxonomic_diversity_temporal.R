...
traits_kelp$Size <- as.ordered(traits_kelp$Size)
ordered(traits_kelp$Size)
traits_kelp$Agg <- as.ordered(traits_kelp$Agg)
ordered(traits_kelp$Agg)
# I have changed the order of the groups with Solitary < Pair < Group ...
# ... instead of Group < Pair < Solitary:
traits_kelp[, "Agg"] <- factor(traits_kelp[, "Agg"], levels = c("Solitary", "Pair", "Group"),
                         ordered = TRUE)
ordered(traits_kelp$Agg)
traits_kelp$Position <- as.ordered(traits_kelp$Position)
ordered(traits_kelp$Position)
traits_kelp$Diet <- as.factor(traits_kelp$Diet)

#Thermal affinity data to add later 

#sp_tr$Thermal_affinity_min <- as.numeric(sp_tr$Thermal_affinity_min)
#sp_tr$Thermal_affinity_max <- as.numeric(sp_tr$Thermal_affinity_max)
